...
O repositório `structured_logger` migra de pacote Flutter na raiz para monorepo com `packages/structured_logger` (Dart puro, v1.0.0), integrações em `packages/`, apps em `apps/`, orquestrado por Pub Workspaces + Melos 8.x. Flutter permanece consumidor, não dependência do core.
